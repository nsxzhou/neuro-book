...
- Proposal accepted 后新建或重新打开 Task 时，Leader 必须填写 `agentWorkflow`；Tasker 开始前读取 `.agents/skills/agent-workflow-router/SKILL.md` 和该画像，按 `verification.required` 建立验证回路，并为每个 `verification.notRun` 保留具体授权或环境原因。历史 Task 可暂不补写，除非重新打开或继续执行。
