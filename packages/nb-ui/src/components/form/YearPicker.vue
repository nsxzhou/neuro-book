...
<template>
    ...
    >
        <YearPickerHeader class="flex items-center justify-between pb-2.5">
            <YearPickerPrev class="nb-ui-focus-ring flex h-7 w-7 items-center justify-center rounded-[var(--radius-control)] text-[var(--text-secondary)] transition-colors [transition-duration:var(--motion-fast)] [transition-timing-function:var(--ease-standard)] hover:bg-[color-mix(in_srgb,var(--text-main)_10%,transparent)] hover:text-[var(--text-main)] cursor-pointer disabled:cursor-not-allowed disabled:opacity-40">
                <span class="i-lucide-chevron-left h-4 w-4" aria-hidden="true" />
            </YearPickerPrev>

            <YearPickerHeading class="text-[var(--text-sm)] font-semibold text-[var(--text-main)]" />

            <YearPickerNext class="nb-ui-focus-ring flex h-7 w-7 items-center justify-center rounded-[var(--radius-control)] text-[var(--text-secondary)] transition-colors [transition-duration:var(--motion-fast)] [transition-timing-function:var(--ease-standard)] hover:bg-[color-mix(in_srgb,var(--text-main)_10%,transparent)] hover:text-[var(--text-main)] cursor-pointer disabled:cursor-not-allowed disabled:opacity-40">
                <span class="i-lucide-chevron-right h-4 w-4" aria-hidden="true" />
            </YearPickerNext>
        </YearPickerHeader>

        <div class="flex flex-col gap-2 pt-1">
            <YearPickerGrid class="w-full border-collapse space-y-1">
                <YearPickerGridBody class="space-y-1">
                    <YearPickerGridRow
