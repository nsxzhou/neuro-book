...
const [directory, sessionIdText, expectedVersionText, valueText, startFile, readyFile] = Bun.argv.slice(2);
if (!directory || !sessionIdText || !expectedVersionText || !valueText || !startFile || !readyFile) {
    throw new Error("jsonl-commit-worker 参数不完整");
}
...
try {
    ...
    const result = await new JsonlSessionStore({directory}).commit({
        ...
        cause: `test.process.worker.${value}`,
